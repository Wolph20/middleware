...
data = pd.read_csv("./traza_1000.csv", names=['arrive_time', 'worker_id', 'petition_type', 'memory_size', 'portion'], header=None, sep=" ")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 5:
        ...
        for i in samples:
            ...
            subprocess.call("echo Samples: " + str(i) + " ; echo Write: " + str(write) + " ; echo Read: " + str(read) + " ; echo Number of workers: " + str(id_worker), shell=True)
            
            subprocess.call('./QueueSimulator/single ' + str(inter_arrive_meanTime) + ' ' +
            service_time + ' ' + str(operation_number) + " ; echo Total Size: " + str(size_storage) + " ; echo \n", shell=True)
